...
Every `update`/`delete`/`move` verb takes a UUID `-id`, not a name. Run the matching `list`
first (`account list`, `group list`, `stock list -group <ref>`, `deposit list`,
`holding list -account <name>`), find the row, and take its `"ID"` field. Never guess or
fabricate a UUID.
...
holdings too) are not reversible from the CLI. Confirm with the user before running any
