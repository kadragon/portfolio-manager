...
- [x] [feat] `KIS_APP_KEY_2` per-account balance sync — ported in #103/#107; `container.Container` exposes `AccountSyncByKeyID map[int64]*services.KisAccountSyncService`. Note: `PriceSync` and `OrderClient` still use the primary key only. (`internal/container/container.go`)
