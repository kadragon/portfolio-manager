...
### Decision/Learning
웹 UI CSS 프레임워크를 Pico CSS에서 Tailwind CSS v4 (standalone CLI) + DaisyUI v5로 전환했다.

### Reason
Pico CSS는 미니멀 프레임워크로 컴포넌트가 부족하여 미려한 디자인을 구현하기 어려웠다. Tailwind standalone CLI는 Node.js 없이 단일 바이너리로 동작하므로 Python 프로젝트에 적합하다.

### Impact
- CSS 빌드: `make setup`으로 Tailwind CLI + DaisyUI 다운로드, `make css-watch`/`make css-build`로 빌드.
- 커스텀 테마: `src/portfolio_manager/web/tailwind/input.css`에 `[data-theme="portfolio"]`로 정의 (oklch 색상).
- 템플릿: 전체 25개 Jinja2 템플릿이 DaisyUI 컴포넌트 클래스 사용 (navbar, stats, table, card, badge, alert, btn 등).
- JS: `app.js`에 DaisyUI toast 알림 (자동 dismiss 3초) 추가.
- HTMX 애니메이션: `input.css`에 `.htmx-swapping`/`.htmx-settling` 전환 효과 정의.
- `bin/` 디렉토리와 `daisyui.css`는 `.gitignore`에 추가 (다운로드 바이너리/번들).
