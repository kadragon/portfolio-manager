...
def test_get_stock_change_rates_supports_custom_periods_order():
    """요청한 기간 순서(1D/1M/1Y)대로 변동률 키를 반환한다."""
    price_client = Mock()
    price_client.get_price.return_value = PriceQuote(
        symbol="AAPL", name="Apple Inc.", price=120.0, market="US", currency="USD"
    )
    ...
    assert list(change_rates.keys()) == ["1d", "1m", "1y"]
    assert change_rates["1d"] == Decimal("20")
    assert change_rates["1m"] == Decimal("100")
...
def test_get_stock_change_rates_adjusts_1d_to_previous_business_day():
    ...
    price_client.get_historical_close.side_effect = (
        lambda ticker, target_date, preferred_exchange=None: {
            date(2025, 1, 10): 100.0,  # 2025-01-12 (Sun) -> 2025-01-10 (Fri)
        }[target_date]
    )
    ...
    assert change_rates["1d"] == Decimal("20")
    price_client.get_historical_close.assert_called_once_with(
        "AAPL", date(2025, 1, 10), preferred_exchange=None
    )
