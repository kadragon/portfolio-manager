...
### 현재 상태
- 버튼 클릭 방식: 마우스 또는 Tab 키로 버튼에 포커스 후 Enter
- 메뉴 선택 방식: 버튼 위젯 사용

### 목표
- **모든 기능을 키보드만으로 제어 가능하도록 개선**
- 마우스 없이도 완전히 사용 가능한 TUI
...
#### 2. 그룹 관리 화면 키보드 단축키
- [x] `↑/↓` 키: 그룹 목록 탐색 (이미 구현됨)
- [x] `Enter` 키: 선택한 그룹의 스톡 보기 (이미 구현됨)
- [x] `Delete` 키: 선택한 그룹 삭제 (이미 구현됨)
- [ ] `a` 키: 그룹 추가 (버튼 대신)
- [ ] `Escape` 키: 메인 화면으로 돌아가기

#### 3. 스톡 관리 화면 키보드 단축키
- [x] `↑/↓` 키: 스톡 목록 탐색 (이미 구현됨)
- [x] `Delete` 키: 선택한 스톡 삭제 (이미 구현됨)
- [ ] `a` 키: 스톡 추가 (버튼 대신)
- [ ] `Escape` 키: 그룹 목록으로 돌아가기

#### 4. 입력 화면 키보드 단축키
- [x] `Enter` 키: 입력 완료 및 저장 (이미 구현됨)
- [ ] `Escape` 키: 입력 취소

#### 5. UI 개선
- [ ] 화면 하단에 사용 가능한 키 힌트 표시 (Footer)
  - 예: `g:그룹 관리 | q:종료`
  - 예: `↑↓:탐색 | Enter:선택 | Del:삭제 | a:추가 | Esc:뒤로`
- [ ] 버튼을 Label 또는 Text로 교체하여 키보드 힌트만 표시

### TDD 진행 순서
1. 메인 화면 `g` 키 테스트 작성 → 구현
2. 그룹 화면 `a` 키 테스트 작성 → 구현
3. 스톡 화면 `a` 키 테스트 작성 → 구현
4. `Escape` 키 네비게이션 테스트 작성 → 구현
5. Footer 키 힌트 표시 테스트 작성 → 구현
