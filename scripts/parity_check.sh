...
if [ -n "$AID" ]; then
  ...
  curl -s -o "$OUT/py_holdings_page.html" "http://127.0.0.1:$PY_PORT/accounts/$AID/holdings"
  if [ -n "$HID" ]; then
...
if [ -n "$AID" ]; then
  ...
  curl -s -o "$OUT/go_holdings_page.html" "http://127.0.0.1:$GO_PORT/accounts/$AID/holdings"
  if [ -n "$HID" ]; then
...
def norm(p):
    ...
    # Python list_holdings omits account_id from _holding_row context so delete
    # URLs become /accounts//holdings/... (Python bug). Normalize both sides by
    # replacing the account segment with a placeholder.
    s = re.sub(r'hx-delete="/accounts/[^/]*/holdings/', 'hx-delete="/accounts/ACCOUNT/holdings/', s)
    return s.strip()
rc = 0
