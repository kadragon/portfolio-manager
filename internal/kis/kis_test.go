...
func TestDomesticOrderClientPlaceOrderLimit(t *testing.T) {
	var received map[string]string
	client, baseURL := makeClient(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if err := json.NewDecoder(r.Body).Decode(&received); err != nil {
			t.Errorf("decode body: %v", err)
	...
	if _, err := oc.PlaceOrder(context.Background(), "360750", "buy", 642, "", "27470"); err != nil {
		t.Fatalf("PlaceOrder limit: %v", err)
	}
	if received["ORD_DVSN"] != "00" {
		t.Errorf("ORD_DVSN = %q, want 00 (limit)", received["ORD_DVSN"])
	}
	if received["ORD_UNPR"] != "27470" {
		t.Errorf("ORD_UNPR = %q, want 27470", received["ORD_UNPR"])
	}
}
