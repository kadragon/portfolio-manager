...
func (h *DashboardHandler) index(c echo.Context) error {
	...
	if h.c.Portfolio != nil && h.c.Portfolio.HasPriceService() {
		...
		if err == nil {
			return templates.DashboardPage(summary, nil, "").Render(ctx, c.Response().Writer)
		}
		if !errors.Is(err, services.ErrNoPriceService) {
			...
			return templates.DashboardPage(nil, groupHoldings, err.Error()).Render(ctx, c.Response().Writer)
		}
	}

	// No price service or not configured → group_holdings fallback.
	if h.c.Portfolio == nil {
		return templates.DashboardPage(nil, nil, "").Render(ctx, c.Response().Writer)
	}
	groupHoldings, err := h.c.Portfolio.GetHoldingsByGroup(ctx)
	if err != nil {
		return c.NoContent(http.StatusInternalServerError)
	}
	return templates.DashboardPage(nil, groupHoldings, "").Render(ctx, c.Response().Writer)
}
