...
func (h *DashboardHandler) index(c echo.Context) error {
	...
	if h.c.Portfolio != nil && h.c.Portfolio.HasPriceService() {
		...
		if err == nil {
			accounts, _ := h.c.Accounts.ListAll(ctx)
			return templates.DashboardPage(summary, nil, accounts, "").Render(ctx, c.Response().Writer)
		}
		if !errors.Is(err, services.ErrNoPriceService) {
			...
			return templates.DashboardPage(nil, groupHoldings, nil, err.Error()).Render(ctx, c.Response().Writer)
		}
	}
	...
	return templates.DashboardPage(nil, groupHoldings, nil, "").Render(ctx, c.Response().Writer)
}
