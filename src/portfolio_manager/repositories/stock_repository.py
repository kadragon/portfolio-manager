...
class StockRepository:
    ...
    def list_by_group(self, group_id: UUID) -> list[Stock]:
        ...
        return [
            self._to_stock(item) for item in cast(list[dict[str, Any]], response.data)
        ]
    ...
    @staticmethod
    def _to_stock(item: dict[str, Any]) -> Stock:
        return Stock(
            id=UUID(str(item["id"])),
