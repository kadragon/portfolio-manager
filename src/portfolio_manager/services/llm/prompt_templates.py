...
규칙:
1. 사용자가 제공한 JSON 수치를 그대로 인용하세요. 새 숫자를 만들거나 반올림 규칙을 바꾸지 마세요.
2. 과장·단정을 피하고 "~로 보입니다", "~한 경향이 있습니다" 같은 관찰 어조를 사용하세요.
3. 투자 권유·예측을 하지 말고 현재 상태 관찰과 리스크 포인트만 정리하세요.
4. 4~6문장, 단락 하나로 작성하세요.
5. 출력에 머리말·꼬리말·마크다운 헤더 없이 본문만 출력하세요."""
...
4. 반드시 JSON 으로만 응답하세요. 형식:
   {"summary": "전체 1~2문단 요약", "items": [{"priority": <int>, "rationale": "..."}]}
5. items 는 입력의 priority 와 순서를 유지하고 모든 추천을 포함해야 합니다."""
...
def narrative_user_prompt(payload_json: str, period: str) -> str:
    ...
    return (
        f"다음은 포트폴리오 {period_label} 스냅샷입니다. 규칙대로 4~6문장으로 요약하세요.\n\n"
        f"```json\n{payload_json}\n```"
    )
...
def rebalance_xai_user_prompt(payload_json: str) -> str:
    return (
        ...
        "items 는 입력의 priority·순서를 그대로 유지해야 합니다.\n\n"
        f"```json\n{payload_json}\n```"
    )
