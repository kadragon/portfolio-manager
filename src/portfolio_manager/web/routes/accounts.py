...
def _build_stock_name_map(container, stocks: list | None = None) -> dict:
    ...
    stock_service = getattr(container, "stock_service", None)
    if stock_service is None:
        return {}

    stock_name_map = {}
    for stock in stock_items:
        name = stock_service.resolve_and_persist_name(stock)
        if name:
            stock_name_map[stock.id] = name
...
def sync_account(
    request: Request,
    account_id: UUID,
    confirm_empty: bool = Form(default=False),
) -> HTMLResponse:
    container = get_container(request)
    templates = get_templates(request)
    ...
    try:
        ...
        result = container.kis_account_sync_service.sync_account(
            ...
            allow_empty_snapshot=confirm_empty,
        )
        success = True
        message = "KIS 계좌 동기화 완료"
        show_confirm_empty_button = False
    except KisEmptySnapshotError as exc:
        result = None
        message = f"동기화 중단: {exc}"
        success = False
        show_confirm_empty_button = not confirm_empty
    except KisApiBusinessError as exc:
        result = None
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "message": message,
            "result": result,
            "show_confirm_empty_button": show_confirm_empty_button,
        },
    )
